...
class RequestHandler : public xEventHandler
{
public:
	...
	RequestHandler(handle_t handle) :
	  ...
	  virtual void HandleWrite()
	  {
		  ...
		  if (len > 0)
		  {
			  ...
			  g_reactor.RegisterHandler(this, xReadEvent);
		  }
		  else
		  {
	  ...
	  virtual void HandleRead()
	  {
		  ...
		  if (len > 0)
		  {
			  ...
			  {
				  g_reactor.RegisterHandler(this, xWriteEvent);
			  }
...
class TimeServer : xEventHandler
{
public:
	...
	TimeServer(const char * ip, unsigned short port) :
	  ...
	  virtual void HandleRead()
	  {
		  ...
		  else
		  {
			  ...
			  if (g_reactor.RegisterHandler(handler, xReadEvent) != 0)
			  {
...
int main(int argc, char ** argv)
{
	...
	xheaptimer* printtask = new xheaptimer(5);
	...
	while (1)
	{
		g_reactor.RegisterHandler(&server, xreactor::kReadEvent);
		g_reactor.start();
	}
	return EXIT_SUCCESS;
}
